...
import logic.utils.ClicksMBean;
import logic.utils.SquareMBean;
...
public class PointController {
    ...
    private final ClicksMBean clicksMBean;
    private final SquareMBean squareMBean;

    @Autowired public PointController(JWTUtil jwtUtil, UserService userService, ClicksMBean clicksMBean,
                                      SquareMBean squareMBean) {
        this.jwtUtil = jwtUtil;
        this.userService = userService;
        this.clicksMBean = clicksMBean;
        this.squareMBean = squareMBean;
    }
    ...
    private ResponseEntity<String> addPoint(@Valid @RequestBody PointDTO data, HttpServletRequest req) {
        ...
        clicksMBean.takeAShot(p.isStatus());
        squareMBean.addSquare(modifiedUser.getEmail(), p.getR());
    ...
    private ResponseEntity<String> clearPoints(HttpServletRequest req) {
        ...
        userService.updateUser(modifiedUser);
        squareMBean.addSquare(modifiedUser.getEmail(), 0);
